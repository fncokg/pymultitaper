...
def batched_signal(
    ...
    Args:
        data_list (list): A list of 1D numpy or cupy arrays.
    ...
    Returns:
        NDArray: A 2D array where each row corresponds to a signal from `data_list`, padded with zeros as necessary.
    """
    if padding_strategy == "max":
        max_len = max(signal.shape[0] for signal in data_list)
